...
function filterByRange(
  ...
  switch (range) {
    ...
    case "week":
      return isSameWeek(date, referenceDate, { weekStartsOn: 0 });
    case "month":
...
export default function MerchantDashboard({
  ...
  const [subsidiaryAccountNumber, setSubsidiaryAccountNumber] = useState<
    string | null
  >(null);
  ...
  type PaymentFlowPhase =
    ...
    | "awaiting_pin"
    | "link_submitting";
  const [paymentFlowPhase, setPaymentFlowPhase] =
    useState<PaymentFlowPhase>("idle");
  ...
  const [itemPickerStyle, setItemPickerStyle] =
  ...
  const composeDescription = (nextCart: CartLine[], note: string) => {
    const itemsText = nextCart
      .map((line) => `${line.name} x${line.qty}`)
      .join(", ");
    return [itemsText, note.trim()]
      .filter(Boolean)
      .join(", ")
  ...
  const applyCartToForm = useCallback((nextCart: CartLine[], note: string) => {
    setRequestForm((prev) => {
      ...
      return {
        ...
        amount:
          nextCart.length > 0 ? String(Math.round(total * 100) / 100) : "",
      };
    });
  }, []);
      ...
      const next =
        qty <= 0
          ...
          : exists
            ...
            : [
                ...prev,
                { itemId: item.id, name: item.name, price: item.price, qty },
              ];
  ...
  const stats = useMemo(() => {
    ...
    const currentTxs = successfulTxs.filter((tx) =>
      filterByRange(new Date(tx.timestamp), timeRange, now),
    );
    const currentTotal = currentTxs.reduce((acc, tx) => acc + tx.amount, 0);

    const prevRef = getPreviousReferenceDate(timeRange, now);
    const prevTxs = successfulTxs.filter((tx) =>
      filterByRange(new Date(tx.timestamp), timeRange, prevRef),
    );
    const prevTotal = prevTxs.reduce((acc, tx) => acc + tx.amount, 0);

    const trend =
      prevTotal > 0
        ? Math.min(((currentTotal - prevTotal) / prevTotal) * 100, 100)
        : 0;
    const trendLabel =
      prevTotal > 0
        ? Math.abs(trend) >= 100
  ...
  const successTransaction = useMemo(
    () =>
      transactions.find(
        (t) => t.transactionReference === generatedResult?.transactionReference,
      ) ?? null,
  ...
  const recentActivityPageCount = Math.max(
    1,
    Math.ceil(recentActivityTransactions.length / RECENT_ACTIVITY_PAGE_SIZE),
  );
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (tRes.ok) {
          ...
          const live = sorted.some((tx: { status: TransactionStatus }) =>
  ...
  const stopPushPolling = useCallback(() => {
    if (pushPollRef.current.intervalId)
      clearInterval(pushPollRef.current.intervalId);
    if (pushPollRef.current.timeoutId)
      clearTimeout(pushPollRef.current.timeoutId);
      ...
      pushPollRef.current.intervalId = setInterval(async () => {
        try {
          const res = await fetch(
            `/api/merchants/${id}/transactions/${transactionReference}`,
          );
          if (!res.ok) return;
          ...
          if (tx.status === "success" || tx.status === "failed") {
            ...
            if (tx.status === "success") {
              toast({
                ...
                description:
                  "The customer completed authorization on their phone.",
              });
              setRequestForm((prev) => ({
                ...prev,
                amount: "",
              ...
              toast({
                ...
                description:
                  "The provider reported this payment did not complete.",
              });
            }
    ...
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-muted/20 gap-4">
        ...
        <p className="text-muted-foreground">
          The requested merchant account does not exist.
        </p>
      </div>
    );
  }
        ...
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-slate-500">
              Your merchant account is currently{" "}
              <span className="font-semibold text-orange-600 uppercase text-sm">
          ...
          <Card className="border-orange-100 bg-orange-50/30 text-left">
            <CardContent className="p-4 flex gap-3">
              ...
              <div className="text-sm text-orange-800">
                ...
                <p>
                  Full access to the merchant portal and payment features is
                  only granted once your registration has been fully approved by
                  the compliance team.
                </p>
              </div>
            </CardContent>
  ...
  const handleRequestPayment = async (
    mode: "push" | "link",
    retryData?: { amount: string; phone: string },
  ) => {
    ...
    const isValidEthiopianPhone = /^(?:\+251|251|09)\d{7,10}$/.test(
      trimmedPhone,
    );
    if (!isValidEthiopianPhone) {
      toast({
        ...
        title: "Invalid Phone",
        description:
          "Please enter a valid Ethiopian phone number (e.g., 0912345678, +251..., or 251...).",
      });
      return;
    }
    ...
    setPaymentFlowPhase(
      mode === "push" ? "push_submitting" : "link_submitting",
    );
    ...
    try {
      ...
      const endpoint =
        mode === "push" ? "/api/payments/push" : "/api/payments/link";
      ...
      if (mode === "push") {
        const transactionReference = data?.transactionReference as
          | string
          | undefined;
        ...
        if (transactionReference) {
          ...
          toast({
            ...
            description:
              "Payment was accepted but no transaction reference was returned.",
          });
        }
        return;
      }

      const linkTransactionReference = data?.transactionReference as
        | string
        | undefined;
      setGeneratedResult({
  ...
  const isPending =
    merchant.status === "pending" || merchant.status === "branch_approved";
  const isApproved =
  ...
  const handleMpgsAction = async (action: "qr" | "send") => {
    ...
    const cached =
      mpgsLink && mpgsLink.signature === signature ? mpgsLink : null;
    ...
    try {
      ...
      const payload = {
        ...
        transactionId,
        userCredentials: {
          authToken: `demo_auth_${Math.random().toString(36).slice(2, 10)}`,
        },
        amount: amountNum,
        serviceDescription:
          requestForm.description || "Payment Request for Customer",
      ...
      <div className="flex flex-col h-full">
        <div className="flex-1 overflow-y-auto">
          <div className="px-5 py-3 space-y-4">
            <div
              className={`space-y-4 ${isFormLocked ? "pointer-events-none opacity-60" : ""}`}
            >
              <div className="space-y-2">
                ...
                <RadioGroup
                  ...
                  onValueChange={(val) =>
                    setRequestForm({
                      ...requestForm,
                      method: val as "BANK" | "TELEBIRR" | "MPGS",
                    })
                  }
                  ...
                  >
                    {cart.map((line) => (
                      ...
                      >
                        ...
                        <span className="min-w-[10px] text-center font-semibold">
                          {line.qty}
                        </span>
                        <button
                          type="button"
                    ...
                    <div className="flex min-w-[100px] flex-1 items-center gap-1.5">
                      ...
                      <input
                        ...
                        placeholder={
                          cart.length === 0
                            ? "Order #1022, or search your items…"
                            : "Add more…"
                        }
                                  ...
                                  <label className="flex flex-1 min-w-0 items-center gap-2 cursor-pointer">
                                    <Checkbox
                                      ...
                                      onCheckedChange={(checked) =>
                                        updateCartQty(item, checked ? 1 : 0)
                                      }
                                    />
                                    <span className="truncate text-slate-700">
                                      {item.name}{" "}
                                      <span className="font-semibold text-[#754319]">
  ...
  return (
    <div className="space-y-4 pb-24 md:pb-4">
      <section className="rounded-3xl border border-amber-200/30 bg-white/80 p-4 sm:p-5 md:p-6 shadow-xl backdrop-blur-md">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-3">
            <div>
              <p className="text-[9px] md:text-[10px] uppercase tracking-[0.2em] text-amber-700/70">
                Merchant Dashboard
              </p>
              <div className="mt-1.5 md:mt-2 flex items-center gap-2 md:gap-3">
                ...
                <div className="flex items-center gap-1 md:gap-1.5 px-2 md:px-2.5 py-0.5 md:py-1 rounded-full bg-emerald-50 border border-emerald-100/50">
                  ...
                  <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-wider text-emerald-700">
                    Verified
                  </span>
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3">
              <div
                ...
                onClick={() => copyText(merchant.accountNumber, "acc")}
              >
                <div className="flex items-center gap-1.5 md:gap-2 px-2.5 md:px-3 py-1 md:py-1.5 rounded-xl bg-amber-50/50 border border-amber-100/50 transition-all hover:bg-amber-100/50 hover:border-amber-200/50">
                  ...
                  <span className="text-xs md:text-sm font-bold text-amber-900/80 font-mono tracking-wider">
                ...
                <span className="text-[9px] md:text-[10px] font-bold text-amber-700/40 uppercase tracking-tight group-hover:text-amber-700/60 transition-colors">
                  Settlement Account
                </span>
              </div>
              ...
              {subsidiaryAccountNumber && (
                <div
                  className="flex items-center gap-1.5 md:gap-2.5 group cursor-pointer"
                  onClick={() => copyText(subsidiaryAccountNumber, "sub")}
                >
                  <div className="flex items-center gap-1.5 md:gap-2 px-2.5 md:px-3 py-1 md:py-1.5 rounded-xl bg-amber-50/50 border border-amber-100/50 transition-all hover:bg-amber-100/50 hover:border-amber-200/50">
      ...
      {!isApproved && (
        <Card
          className={`mt-4 rounded-3xl border ${isPending ? "border-amber-200 bg-amber-50/90" : "border-rose-200 bg-rose-50/90"}`}
        >
            ...
            <div className="relative flex items-start justify-between">
              ...
              <div>
                <p className="font-semibold text-sm">
                  Account status:{" "}
                  {merchant.status === "branch_approved"
                    ? "Initial Review OK"
                    : merchant.status}
                </p>
                <p className="text-xs text-muted-foreground">
                  {isPending
                    ? "Payment requests unlock once your account is approved."
          ...
          <Card className="card-honey-glass overflow-hidden rounded-3xl border-amber-200/20 bg-white/80 shadow-xl backdrop-blur-md transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:bg-white/90">
            <CardContent className="p-6">
              <div className="flex flex-col">
                <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-amber-800/60 mb-1">
                  Revenue
                </p>
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl font-black text-[#5b371f]">
                    {stats.currentTotal.toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                    })}
                  </span>
                  <span className="text-sm font-bold text-amber-800/40">
                    ETB
                  </span>
                </div>
              </div>
        ...
        <Card className="rounded-3xl border-amber-200/30 bg-white/80 shadow-xl backdrop-blur-sm">
          <CardContent className="p-4 sm:p-5">
            <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="font-semibold text-[#5b371f] text-lg">
                  Recent Activity
                </h2>
                <p className="text-xs text-amber-800/60 leading-5">
                  {recentActivityTransactions.length} transaction
                  {recentActivityTransactions.length === 1 ? "" : "s"}{" "}
                  {timeRange === "today" ? "today" : `this ${timeRange}`}
                </p>
              </div>
              <Link
                href={`/merchant/${id}/transactions`}
                className="inline-flex min-h-11 items-center text-sm font-medium text-amber-700"
              >
            ...
            <div className="space-y-2">
              {recentTransactions.map((tx) => (
                <div
                  key={tx.id}
                  className="group flex flex-col gap-3 rounded-2xl border border-amber-200/30 bg-gradient-to-r from-amber-50/50 to-white/50 p-3 transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-amber-200/20 sm:flex-row sm:items-center sm:justify-between"
                >
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-[#5b371f] break-words">
                      {tx.description}
                    </p>
                    <p className="text-xs leading-5 text-amber-800/60 break-words">
                      {tx.payerPhone || "Web checkout"} •{" "}
            ...
            {recentActivityPageCount > 1 && (
              <div className="mt-3 flex items-center justify-between pt-3 border-t border-amber-100/50">
                <Button
                  ...
                  onClick={() =>
                    setRecentActivityPage((p) => Math.max(0, p - 1))
                  }
                ...
                <Button
                  ...
                  onClick={() =>
                    setRecentActivityPage((p) =>
                      Math.min(recentActivityPageCount - 1, p + 1),
                    )
                  }
                  disabled={
                    safeRecentActivityPage >= recentActivityPageCount - 1
                  }
                >
      ...
      <Sheet
        open={isMobile && isRequestPanelOpen}
        onOpenChange={handleRequestPanelOpenChange}
      >
        <SheetContent
          side="bottom"
        ...
        >
          ...
          <SheetHeader className="text-left mb-3">
            <SheetTitle className="text-xl text-[#5b371f]">
              Request payment
            </SheetTitle>
      ...
      >
        ...
        >
          <DialogHeader className="text-center p-5 border-b border-slate-50 shrink-0">
            ...
            <DialogTitle className="text-lg font-bold text-slate-800 leading-none">
              Request payment
            </DialogTitle>
          ...
          <div className="p-5 text-center border-b border-slate-50">
            {lastMode === "link" ? (
              ...
              <>
                ...
                <h3 className="text-lg font-bold text-emerald-800 tracking-tight leading-none">
                  Payment Successful
                </h3>
                <p className="text-slate-500 mt-1.5 text-[11px]">
              ...
              <>
                ...
                <h3 className="text-lg font-bold text-rose-800 tracking-tight leading-none">
                  Payment Failed
                </h3>
                <p className="text-slate-500 mt-1.5 text-[11px]">
          ...
          <div className="p-5 space-y-4 bg-slate-50/50 overflow-y-auto">
            {lastMode === "link" && generatedResult?.method === "MPGS" ? (
                ...
                {mpgsView === "qr" ? (
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-[11px] text-slate-500 text-center leading-relaxed px-2">
                      ...
                      to open the secure card payment page.
                    </p>
                  </div>
                ) : (
                  ...
                  <div className="relative group">
                    ...
                    <Button
                      onClick={() =>
                        ...
                        copyText(generatedResult.paymentUrl, "modalCopy")
                      }
              ...
              <div className="space-y-4">
                <div
                  className={`flex flex-col items-center justify-center gap-1.5 p-5 rounded-xl border shadow-sm ${
                    currentTxStatus === "success"
                      ? "bg-emerald-50/50 border-emerald-100"
                      : currentTxStatus === "failed"
                        ? "bg-rose-50/50 border-rose-100"
                ...
                >
                  ...
                  >
                    {parseFloat(lastRequestDetails?.amount || "0").toFixed(2)}{" "}
                    ETB
                  </p>
              ...
              <p className="text-[9px] text-center text-slate-400 leading-relaxed">
                {lastMode === "link"
                  ...
                  : currentTxStatus === "success"
                    ...
                    : "Status updates in recent activity log."}
                <br />
