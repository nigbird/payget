'use client'

export const dynamic = 'force-dynamic'

import { useState, useEffect, useMemo } from 'react'
import { useAuth } from '@/lib/auth-context'
import { AlertCircle } from 'lucide-react'
import { Card, CardContent } from '@/components/ui/card'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { PaymentReconciliationTab } from '@/components/reconciliation/payment-reconciliation-tab'
import { CashbackReconciliationTab } from '@/components/reconciliation/cashback-reconciliation-tab'
import { MpgsReconciliationTab } from '@/components/reconciliation/mpgs-reconciliation-tab'
import { ApprovalsQueue } from '@/components/reconciliation/approvals-queue'

/**
 * Unified reconciliation console.
 *
 * Payments, cashback, and card (MPGS) transactions are all repair points on
 * one pipeline, so they live behind one entry point, with a single approvals
 * inbox spanning all three.
 *
 * Tabs are gated on the permissions the user actually holds.
 */
export default function ReconciliationPage() {
  const { user } = useAuth()

  const permissions = useMemo(() => user?.permissions || [], [user])

  const canViewPayments = permissions.includes('payment.reconciliation.view')
  const canViewCashback = permissions.includes('cashback.reconciliation.view')
  const canViewMpgs = permissions.includes('mpgs.reconciliation.view')
  const canManagePayments = permissions.includes('payment.reconciliation.manage')
  const canManageCashback = permissions.includes('cashback.reconciliation.manage')
  const canManageMpgs = permissions.includes('mpgs.reconciliation.manage')

  const canApproveAnything = canManagePayments || canManageCashback || canManageMpgs

  const defaultTab = canViewPayments ? 'payments' : canViewCashback ? 'cashback' : canViewMpgs ? 'mpgs' : 'approvals'
  const [tab, setTab] = useState(defaultTab)

  useEffect(() => {
    setTab(defaultTab)
  }, [defaultTab])

  if (!canViewPayments && !canViewCashback && !canViewMpgs) {
    return (
      <div className="p-8">
        <Card>
          <CardContent className="flex items-center gap-3 p-6 text-muted-foreground">
            <AlertCircle className="h-5 w-5" />
            You do not have permission to view reconciliation.
          </CardContent>
        </Card>
      </div>
    )
  }

  return (
    <div className="space-y-6 p-6">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-[#1F2937]">Reconciliation</h1>
        <p className="mt-1 text-sm text-[#6B7280]">
          Recover payments, cashbacks, and card transactions whose records disagree with the bank
          or gateway.
        </p>
      </div>

      <Tabs value={tab} onValueChange={setTab}>
        <TabsList>
          {canViewPayments && <TabsTrigger value="payments">Push Payment</TabsTrigger>}
          {canViewCashback && <TabsTrigger value="cashback">Cashback</TabsTrigger>}
          {canViewMpgs && <TabsTrigger value="mpgs">Card (MPGS)</TabsTrigger>}
          {canApproveAnything && <TabsTrigger value="approvals">Approvals</TabsTrigger>}
        </TabsList>

        {canViewPayments && (
          <TabsContent value="payments" className="mt-4">
            <PaymentReconciliationTab embedded />
          </TabsContent>
        )}

        {canViewCashback && (
          <TabsContent value="cashback" className="mt-4">
            <CashbackReconciliationTab embedded />
          </TabsContent>
        )}

        {canViewMpgs && (
          <TabsContent value="mpgs" className="mt-4">
            <MpgsReconciliationTab embedded />
          </TabsContent>
        )}

        {canApproveAnything && (
          <TabsContent value="approvals" className="mt-4">
            <ApprovalsQueue
              canManagePayments={canManagePayments}
              canManageCashback={canManageCashback}
              canManageMpgs={canManageMpgs}
              canViewPayments={canViewPayments}
              canViewCashback={canViewCashback}
              canViewMpgs={canViewMpgs}
            />
          </TabsContent>
        )}
      </Tabs>
    </div>
  )
}
