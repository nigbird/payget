...
async function fetchServerPublicKey(): Promise<string> {
  ...
  const cleanBaseUrl = PROVIDER_BASE_URL.endsWith('/') ? PROVIDER_BASE_URL.slice(0, -1) : PROVIDER_BASE_URL
  const url = `${cleanBaseUrl}/nib-push-payment/api/get-pub-key`
...
export async function sendProviderPushRequest(request: PushPaymentRequest): Promise<ProviderResponse> {
  try {
    ...
    const cleanBaseUrl = PROVIDER_BASE_URL.endsWith('/') ? PROVIDER_BASE_URL.slice(0, -1) : PROVIDER_BASE_URL
    const url = `${cleanBaseUrl}/push-payment/transfer`
