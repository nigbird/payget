...
/**
 * Cashback side of the reconciliation console.
 *
 * `embedded` is set when this renders inside /admin/reconciliation, where the
 * page supplies its own heading and the pending requests live in the shared
 * approvals queue rather than in a tab of their own.
 */
export function CashbackReconciliationTab({ embedded = false }: { embedded?: boolean }) {
  const { user } = useAuth()
  ...
  const [requestType, setRequestType] = useState<'REFERENCE_UPDATE' | 'RETRY' | 'MANUAL_SETTLE'>('RETRY')
  ...
  const handleCreateRequest = async () => {
    ...
    if (requestType === 'MANUAL_SETTLE' && !ftNumber.trim()) return
    ...
    try {
      const res = await fetch('/api/admin/cashback-reconciliation', {
        ...
        body: JSON.stringify({
          ...
          ftNumber: requestType === 'MANUAL_SETTLE' ? ftNumber.trim() : null,
  ...
  return (
    <>
      <div className='space-y-6 bg-white'>
        ...
        <Tabs value={embedded ? 'transactions' : undefined} defaultValue='transactions'>
          {!embedded && (
            <TabsList>
                  ...
                  <Dialog open={isRequestDialogOpen} onOpenChange={setIsRequestDialogOpen}>
                    ...
                    <DialogContent>
                      ...
                      <div className='space-y-4 py-4'>
                        <div className='space-y-2'>
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value='MANUAL_SETTLE'>Settle by FT (bank receipt)</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        ...
                        {requestType === 'MANUAL_SETTLE' && (
                          <div className='space-y-2'>
                            <Label>FT Number</Label>
                            <Input
                              value={ftNumber}
                            ...
                            <p className='text-xs text-muted-foreground'>
                              Marks the cashback completed against this FT without calling the
                              provider. Requires approval by a different user.
                            </p>
                          </div>
                        )}
                        <div className='space-y-2'>
